...
config_file_paths = [
    './imap2html.ini',
    './.imap2html.ini',
    '~/.config/imap2html.ini',
    '/opt/local/etc/imap2html.ini',
    '/etc/imap2html.ini'
]
...
IMAPSERVER = config.get('imap2html', 'imap_server')
IMAPLOGIN = config.get('imap2html', 'imap_user')
IMAPPASSWORD = config.get('imap2html', 'imap_password')

if IMAPPASSWORD == "":
    IMAPPASSWORD = getpass.getpass()

IMAPFOLDER_ORIG = [folder.strip() for folder in \
                   config.get('imap2html', 'imap_folder').split(',') \
                   if folder.strip() != ""]
...
ssl = False
try:
    ssl_value = config.get('imap2html', 'ssl')
...
try:
    incremental_value = config.get('imap2html', 'incremental_backup')
...
offline = False
try:
    offline_value = config.get('imap2html', 'offline')
...
subfolders = False
try:
    subfolders_value = config.get('imap2html', 'subfolders')
    if subfolders_value in yes_flags:
        subfolders = True
except:
    pass
...
CreateMailDir = True
try:
    CreateMailDir_value = config.get('imap2html', 'MailDir')
    if not CreateMailDir_value in yes_flags:
        CreateMailDir = False
except:
    pass
...
destFolder = "./"
try:
    destFolder = config.get('imap2html', 'folder')
except:
    pass

messages_per_overview_page = 50
try:
    messages_per_overview_page = config.get('imap2html', 'messages')
except:
    pass
...
def returnIndexPage():
    ...
    with open("index.html", "w") as indexFile:
        ...
        indexFile.write("<p>This is your email backup. You've made it with TBC<br />\n")
        ...
        for dfolder in IMAPFOLDER:
            indexFile.write("\t<li><a href = \"" + dfolder + "/email-report-1.html\">" + dfolder + "</a></li>\n")
        indexFile.write("</ul>\n")
        indexFile.write("<br />Available Folders:<br />\n<ul>\n")
        if not offline:
            for dfolder in IMAPALLFOLDER:
                indexFile.write("\t<li>" + dfolder + "</li>\n")
            indexFile.write("</ul>\n")
        if ssl:
            indexFile.write(
...
def createOverviewPage(folder, pagenumber, amountOfItems=50):
    local_maildir_folder = destFolder + folder
    if not os.path.exists(local_maildir_folder):
        os.makedirs(local_maildir_folder)
    overview_page_name = "email-report-" + str(pagenumber) + ".html"
    overview_file_path = os.path.join(local_maildir_folder, overview_page_name)
...
def addMailToOverviewPage(folder, pagenumber, mail_id, mail_from,
    ...
    overview_page_name = "email-report-" + str(pagenumber) + ".html"
    overview_file_path = os.path.join(destFolder + folder, overview_page_name)
...
def createMailPage(folder,
    ...
    folder_path_1 = os.path.join(destFolder + folder, attachment_folder_date, str(mail_number))

    mail_html_page = os.path.join(folder_path_1, "index.html")
    with open(mail_html_page, 'w') as mail_page:
        mail_page.write(returnHeader(mail_subject, inc_location))
...
# ------------------------------------------------------------------------------
# save_mail_attachments_to_folders
# ------------------------------------------------------------------------------
def save_mail_attachments_to_folders(mail_id, mail, local_folder, folder):
    ...
    if not os.path.exists(os.path.join(destFolder + folder, att_date, str(mail_id), "attachments/")):
        os.makedirs(os.path.join(destFolder + folder, att_date, str(mail_id), "attachments/"))
    else:
        remove(os.path.join(destFolder + folder, att_date, str(mail_id), "attachments/"))
        os.makedirs(os.path.join(destFolder + folder, att_date, str(mail_id), "attachments/"))

    with open(os.path.join(destFolder + folder, att_date, str(mail_id), "attachments/index.html"), "w") as att_index_file:
...
print(      "##############################################")
print(      "# imap2html.py: IMAP Email Backup to HTML    #")
if offline:
...
# create local Maildir

mailbox.Maildir(os.path.join(destFolder + maildir), factory=None, create=True)
...
if not offline:
    ...
    IMAPALLFOLDERS = []
    maillist = mail.list()
    for imapFolder in maillist[1]:
        imapFolder = re.sub(r"(?i)\(.*\)", "", imapFolder, flags=re.DOTALL)
        imapFolder = re.sub(r"(?i)\".\"", "", imapFolder, flags=re.DOTALL)
        imapFolder = re.sub(r"(?i)\"", "", imapFolder, flags=re.DOTALL)
        imapFolder = imapFolder.strip()
        IMAPALLFOLDERS.append(imapFolder)

# saving messages in local Maildir

for mailFolder in IMAPALLFOLDERS:
    if mailFolder in IMAPFOLDER_ORIG:
        if not offline:
            print(("Getting messages from server from folder: %s.") % mailFolder)
            retries = 0
            success = False
            while success == False and retries < 5:
                try:
                    ...
                    folder_most_recent_id = 0
                    if not os.path.exists(destFolder + "imap2html.txt"):
                        with open(os.path.join(destFolder + "imap2html.txt"), 'w') as progress_file:
                            progress_file.write(mailFolder + ":" + IMAPLOGIN + ":1")
                            progress_file.close()
                    match = False
                    with open(os.path.join(destFolder + "imap2html.txt"), 'r') as progress_file:
                        for line in progress_file:
                            if len(line) > 3:
                                latest_mailid = line.split(":")[2]
                                email_addres_from_file = line.split(":")[1]
                                folder_name = line.split(":")[0]
                                if folder_name == mailFolder and email_addres_from_file == IMAPLOGIN:
                                    progress_file.close()
                                    folder_most_recent_id = latest_mailid
                                    break
                        progress_file.close()
                    ...
                    for message_id in range(int(startid), int(total_messages_in_mailbox + 1)):
                        ...
                        mbox = mailbox.Maildir(destFolder + maildir, factory=mailbox.MaildirMessage, create=True)
                        folder = mbox.add_folder(mailFolder)
                        folder.lock()
                        try:
                            message_key = folder.add(raw_email)
                            folder.flush()
                        ...
                        if incremental_backup:
                            match = False
                            for line in fileinput.input(destFolder + "imap2html.txt", inplace=1):
                                if line.split(":")[0] == folder and line.split(":")[1] == IMAPLOGIN and len(
                                        line) > 3:
                                    line = mailFolder + ":" + IMAPLOGIN + ":" + str(message_id)
                                    match = True
                                if len(line) > 3 and line != "\n":
                                    print(line)
                            fileinput.close()
                            if not match:
                                with open(os.path.join(destFolder + "imap2html.txt"), 'a') as progress_file:
                                    progress_file.write(mailFolder + ":" + IMAPLOGIN + ":" + str(message_id))
                                    progress_file.close()
            ...
            if retries == 5:
                ...
                print(("Done with folder: %s.") % mailFolder)
            print("\n")
        print(("Done with folder: %s.") % mailFolder)
    else:
        print(("Skipping folder: %s.") % mailFolder)

for mailFolder in IMAPALLFOLDERS:
    if mailFolder in IMAPFOLDER_ORIG:
        print(("Processing folder: %s.") % mailFolder)

       ## Maildir folders have dots, not slashes
        local_maildir_folder = mailFolder.replace("/", ".")
        local_maildir = mailbox.Maildir(os.path.join(destFolder + maildir), factory=None, create=True)

        try:
            maildir_folder = local_maildir.get_folder(local_maildir_folder)
        except mailbox.NoSuchMailboxError as e:
            print(("Error: Folder \"%s\" is probably empty or does not exists: %s.") % (mailFolder, e))
            createOverviewPage(mailFolder, 1, 0)
            addMailToOverviewPage(mailFolder, 1, 1, "-", "-", "Error: Folder/mailbox does not exist or is empty",
                                  "01-01-1900",
                                  emptyFolder=True)
            finishOverviewPage(mailFolder, 1, 0, 0, 0)
            break
        ...
        for number in reversed(range(number_of_overview_pages)):
            number += 1
            createOverviewPage(mailFolder, number, messages_per_overview_page)
        ...
        for number in reversed(range(start_mail_number)):
            ...
            addMailToOverviewPage(mailFolder, current_page_number, mail_number,
            ...
            # mail_has_attachment = save_mail_attachments_to_folders(mail_number, mail_for_page, mailFolder, mailFolder)
            #createMailPage(mailFolder, mail_number, mail_for_page, current_page_number,
            #               mail_from, mail_to, mail_subject, mail_date,
            #               mail_has_attachment,
            #               mail_from_encoding=mail_from_encoding,
            #               mail_to_encoding=mail_to_encoding,
        ...
        for number in reversed(range(number_of_overview_pages)):
            ...
            if number == 1 and number_of_overview_pages == 1:
                finishOverviewPage(mailFolder, number, 0, 0, total_messages_in_folder)
            elif number == 1:
                finishOverviewPage(mailFolder, number, 0, (number + 1), total_messages_in_folder)
            elif number == number_of_overview_pages:
                finishOverviewPage(mailFolder, number, (number - 1), 0, total_messages_in_folder)
            else:
                finishOverviewPage(mailFolder, number, (number - 1), (number + 1), total_messages_in_folder)
...
if not incremental_backup:
    ...
    now = datetime.datetime.now()
    maildirfilename = destFolder + maildir + "." + str(now).replace("/", ".").replace(" ", ".").replace("-", ".").replace(":", ".")
    move(destFolder + maildir, maildirfilename)
