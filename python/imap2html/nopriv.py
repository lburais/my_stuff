...
subfolders = False
try:
    subfolders_value = config.get('nopriv', 'subfolders')
...
CreateMailDir = True
try:
    CreateMailDir_value = config.get('nopriv', 'MailDir')
...
destFolder = "./"
try:
    destFolder = config.get('nopriv', 'folder')
except:
    pass

messages_per_overview_page = 50
try:
    messages_per_overview_page = config.get('nopriv', 'messages')
except:
    pass
...
# ------------------------------------------------------------------------------
# allFolders
#   get the list of folders to processes
# ------------------------------------------------------------------------------
def allFolders(IMAPFOLDER_ORIG, mail):
    ...
    if len(IMAPFOLDER_ORIG) == 1 and IMAPFOLDER_ORIG[0] == "NoPriv_All":
        ...
        if subfolders:
            for imapFolder in IMAPFOLDER_ORIG:
                maillist = mail.list("", "*" + imapFolder + "*")
                for imapSubFolder in sorted(maillist[1]):
                    imapSubFolder = re.sub(r"(?i)\(.*\)", "", imapSubFolder, flags=re.DOTALL)
                    imapSubFolder = re.sub(r"(?i)\".\"", "", imapSubFolder, flags=re.DOTALL)
                    imapSubFolder = re.sub(r"(?i)\"", "", imapSubFolder, flags=re.DOTALL)
                    imapSubFolder = imapSubFolder.strip()
                    response.append(imapSubFolder)
        else:
...
# ------------------------------------------------------------------------------
# returnImapFolders
#   printable list of folders in html or txt (all available or selected)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# backup_mails_to_html_from_local_maildir
#   TBC
# ------------------------------------------------------------------------------
